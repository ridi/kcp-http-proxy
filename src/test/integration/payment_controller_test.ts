    ...
    it("결제 요청 200 상태 반환", (done) => {        
        ...
        chai.request(app)
            ...
            .end((_, res) => {
                ...
                chai.expect(res.body.code).to.equal("0000");
                chai.expect(res.body.is_success).to.be.true;
                chai.expect(res.body.payment_method).to.equal("PACA");
                chai.expect(res.body.order_no).to.equal(TEST_ORDER_NO);
                chai.expect(res.body.card_code).to.equal("CCLG");
                chai.expect(res.body.card_name).to.equal("신한카드");
                chai.expect(res.body.acquiredee_code).to.equal("CCLG");
                chai.expect(res.body.acquiredee_name).to.equal("신한카드");
                ...
                chai.expect(res.body.is_partial_cancellation).to.be.true;
    ...
    it("결제 취소 200 상태 반환", (done) => {
        chai.request(app)
            ...
            .end((_, res) => {
                ...
                chai.expect(res.body.coupon_amount).to.equal(0);
                chai.expect(res.body.is_escrow).to.be.true;
                chai.expect(res.body.cancellation_gubun).to.equal("B");
                chai.expect(res.body.van_code).to.equal("VNKC");
                chai.expect(res.body.approval_time).to.match(/[0-9]{14}/);
                chai.expect(res.body.van_approval_time).to.match(/[0-9]{14}/);
                chai.expect(res.body.cancellation_time).to.match(/[0-9]{14}/);
                chai.expect(res.body.approval_no).to.match(/[0-9]+/);
