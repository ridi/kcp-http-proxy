...
import { IPaymentRequestRepository } from "../entity/IPaymentRequestRepository";
...
import { IPaymentRequestService } from "./IPaymentRequestService";
...
export class PaymentRequestService implements IPaymentRequestService {
    ...
    async getPaymentAuthKeyResult(mode: Mode, card_number: string, card_expiry_date: string, card_tax_no: string, card_password: string): Promise<PaymentAuthKeyResultEntity | null> {
        ...
        const requestEntity: PaymentRequestEntity | undefined = await this.requestRepository.getPaymentRequest(CommandType.REQUEST_AUTH_KEY, hashed);
    ...
    async getPaymentApprovalResult(mode: Mode, bill_key: string, order_no: string, product_amount: number): Promise<PaymentApprovalResultEntity | null> {
        ...
        const requestEntity: PaymentRequestEntity | undefined = await this.requestRepository.getPaymentRequest(CommandType.PAYMENT_APPROVAL, hashed);
    ...
    async getPaymentCancellationResult(mode: Mode, tno: string): Promise<PaymentCancellationResultEntity | null> {
        ...
        const requestEntity: PaymentRequestEntity | undefined = await this.requestRepository.getPaymentRequest(CommandType.PAYMENT_CANCELLATION, hashed);
